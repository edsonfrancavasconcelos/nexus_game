import { al as Vector3, aj as TrianglesDrawMode, ah as TriangleFanDrawMode, ai as TriangleStripDrawMode, K as Loader, M as LoaderUtils, F as FileLoader, S as MeshPhysicalMaterial, ak as Vector2, i as Color, J as LinearSRGBColorSpace, a8 as SRGBColorSpace, ae as SpotLight, a1 as PointLight, D as DirectionalLight, P as Matrix4, q as InstancedMesh, a5 as Quaternion, p as InstancedBufferAttribute, _ as Object3D, ag as TextureLoader, o as ImageBitmapLoader, e as BufferAttribute, r as InterleavedBuffer, E as LinearMipmapLinearFilter, W as NearestMipmapLinearFilter, H as LinearMipmapNearestFilter, X as NearestMipmapNearestFilter, z as LinearFilter, V as NearestFilter, a7 as RepeatWrapping, U as MirroredRepeatWrapping, g as ClampToEdgeWrapping, a3 as PointsMaterial, N as Material, w as LineBasicMaterial, T as MeshStandardMaterial, m as DoubleSide, R as MeshBasicMaterial, a4 as PropertyBinding, f as BufferGeometry, ab as SkinnedMesh, Q as Mesh, y as LineSegments, L as Line, x as LineLoop, a2 as Points, G as Group, a0 as PerspectiveCamera, O as MathUtils, $ as OrthographicCamera, aa as Skeleton, b as AnimationClip, B as Bone, u as InterpolateDiscrete, v as InterpolateLinear, s as InterleavedBufferAttribute, af as Texture, am as VectorKeyframeTrack, Z as NumberKeyframeTrack, a6 as QuaternionKeyframeTrack, j as ColorManagement, n as FrontSide, t as Interpolant, c as Box3, ac as Sphere, ad as SphereGeometry, Y as NormalBlending, l as CylinderGeometry, A as AdditiveBlending, k as ConeGeometry, d as BoxGeometry, I as IcosahedronGeometry, C as CanvasTexture, a as AmbientLight, a9 as Scene, an as WebGLRenderer, h as Clock } from "./three-CB83ofIZ.js";
(function polyfill() {
...
class Player {
  ...
  fireMissile() {
    ...
    if (this.laserManager && typeof this.laserManager.createMissile === "function") {
      ...
      console.error("ERRO: laserManager ou createMissile não encontrado.");
    }
    return true;
  }
  _updatePDCProjectiles(enemyManager2, dt) {
    ...
    for (let i = this.pdcProjectiles.length - 1; i >= 0; i--) {
      ...
      const waveX = Math.sin(elapsed * 8 + b.offset) * 3;
      const waveY = Math.cos(elapsed * 8 + b.offset) * 3;
  ...
  _initKeyboard() {
    const handleKey = (e, val) => {
      ...
      if (val && !e.repeat && (e.code === "KeyM" || e.code === "KeyA")) this.fireMissile();
    };
  ...
  _shoot() {
    ...
    [this.gunLeft, this.gunRight, this.gunNose].forEach((localPos) => {
      ...
      worldPos.setFromMatrixPosition(new Matrix4().multiplyMatrices(this.shipModel.matrixWorld, new Matrix4().setPosition(localPos)));
  ...
  _emitHeatWash() {
    ...
    for (let k = 0; k < 2; k++) {
      const particleMesh = new Mesh(this.particleGeometry, this.particleMaterial.clone());
      particleMesh.position.set(worldPos.x + (Math.random() - 0.5) * 1.5, worldPos.y + (Math.random() - 0.5) * 1.5, worldPos.z);
      this.scene.add(particleMesh);
      this.particles.push({ mesh: particleMesh, life: 1, speedZ: 180, driftX: (Math.random() - 0.5) * 4, driftY: (Math.random() - 0.5) * 4 });
    }
  }
  update(moveInput, deltaTime, enemyManager2, onPlayerHit = null) {
    ...
    if (this.collisionCooldown > 0) this.collisionCooldown = Math.max(0, this.collisionCooldown - deltaTime);
    ...
    if (((_a = enemyManager2 == null ? void 0 : enemyManager2.enemies) == null ? void 0 : _a.length) && this._isCollidingWithAsteroid(enemyManager2)) {
      this.mesh.position.copy(previousPosition);
      this.velocity.multiplyScalar(0.2);
      this.mesh.updateMatrixWorld();
      if (this.collisionCooldown <= 0 && typeof onPlayerHit === "function") {
        onPlayerHit();
        this.collisionCooldown = 1.1;
      }
    }
    const suavidade = 0.01;
    this.pitch = MathUtils.lerp(this.pitch, moveInput.y * 0.5, suavidade);
    this.roll = MathUtils.lerp(this.roll, -moveInput.x * 0.7, suavidade);
    ...
    for (let i = this.particles.length - 1; i >= 0; i--) {
      ...
      p.life -= dt * 4;
      const currentScale = (1 - p.life) * 4 + 1;
  ...
  _isCollidingWithAsteroid(enemyManager2) {
    ...
    for (const enemy of enemyManager2.enemies) {
      const type = (_a = enemy == null ? void 0 : enemy.userData) == null ? void 0 : _a.type;
      if (type !== "asteroide" && type !== "meteoro") continue;
      enemy.updateMatrixWorld(true);
      const asteroidBox = new Box3().setFromObject(enemy);
...
class EnemyManager {
  ...
  _styleRockModel(model, baseColor = 7301731) {
    model.traverse((child) => {
      if (!child.isMesh || !child.material) return;
      const materials = Array.isArray(child.material) ? child.material : [child.material];
      const styledMaterials = materials.map((material) => {
        ...
        if ("emissive" in nextMaterial) {
          nextMaterial.emissive.setHex(328965);
  ...
  update(laserManager2, onScoreIncrease, player2, deltaTime, explosionManager2, soundManager2, currentLevel = 1) {
    var _a;
    ...
    for (let i = this.enemies.length - 1; i >= 0; i--) {
      ...
      if (foiAtingidoPorLaser && pontoDoImpactoReal) {
        if (explosionManager2) explosionManager2.create(pontoDoImpactoReal);
...
const GEO = {
  debris: new IcosahedronGeometry(3, 1),
  // Reduzido o raio base de 6 para 3
  smoke: new SphereGeometry(5, 8, 8),
  // Reduzido o raio base de 10 para 5 e simplificado segmentos
  // 🛠️ NOVA GEOMETRIA PARA METAIS: Um cone de 3 lados gera estilhaços triangulares pontiagudos e totalmente assimétricos
  shard: new ConeGeometry(2, 6, 3)
};
class ExplosionManager {
  ...
  create(position, options = {}) {
    ...
    const flashColor = options.flashColor || (isMissileExplosion ? 14221240 : 16773341);
    const lightColor = options.lightColor || (isMissileExplosion ? 6750003 : 16755251);
    const lightIntensity = options.lightIntensity || (isMissileExplosion ? 2200 : 1500);
    const smokeColor = options.smokeColor || (isMissileExplosion ? 2047770 : 2236962);
    ...
    const modificadorEscala = proximoAoJogador ? 0.35 : 1;
    const flashGeo = new SphereGeometry(12 * modificadorEscala, 12, 12);
    ...
    const addPart = (geo, color, isSmoke, count, isMetallicDebris = false) => {
      ...
      for (let i = 0; i < contagemFinal; i++) {
        const mat = new MeshBasicMaterial({
          color,
          transparent: true,
          blending: isMetallicDebris ? NormalBlending : AdditiveBlending,
          depthWrite: false
        });
        const mesh = new Mesh(geo, mat);
        let s = isSmoke ? (14 + Math.random() * 10) * modificadorEscala : (5 + Math.random() * 8) * modificadorEscala;
        if (isMetallicDebris) {
        ...
        group.add(mesh);
        const zVelocity = isMetallicDebris ? Math.random() * 35 + 20 : (Math.random() - 0.5) * 25;
        fragments.push({
          mesh,
          isSmoke,
          isMetallicDebris,
          // Marcação para a física e rotação no update
          ...
          // Armazena as escalas individuais deformadas para o efeito do update
    ...
    addPart(GEO.debris, 16777215, false, 15);
    addPart(GEO.debris, 16776960, false, 20);
    addPart(GEO.debris, 16746496, false, 25);
    addPart(GEO.debris, 16720384, false, 20);
    addPart(GEO.debris, 16768358, false, 30);
    addPart(GEO.shard, 1710618, false, 30, true);
    addPart(GEO.shard, 5592405, false, 25, true);
    addPart(GEO.shard, 8947848, false, 15, true);
    ...
    this.explosions.push({
      ...
      fragments,
      life: 1.2,
      // Reduzido o tempo de vida de 2.5 para 1.2 segundos (explosão mais rápida e limpa)
      flashGeo
      // Guardado para descarte de memória correto
    });
  }
  update(deltaTime) {
    if (!deltaTime) return;
      ...
      for (const frag of exp.fragments) {
        ...
        if (frag.isSmoke) {
          ...
          if (frag.isMetallicDebris) {
            const progressoProjecao = 1 + (1.2 - exp.life) * 1.5;
...
class SpaceEnvironment {
  ...
  initGlobe() {
    this.loader.load("/assets/models/planeta.glb", (gltf) => {
      ...
      this.globe.position.set(0, -620, -22e3);
      this.globe.scale.set(28, 28, 28);
      this.scene.add(this.globe);
      console.log("✅ Planeta carregado e posicionado");
    }, void 0, (error) => console.error("Erro ao carregar planeta:", error));
  }
  initParticles() {
    ...
    const cloudMat = new PointsMaterial({
      map: cloudTexture,
      sizeAttenuation: true,
      transparent: true,
      depthWrite: false,
  ...
  applyTheme(theme) {
    ...
    if ((_a = this.clouds) == null ? void 0 : _a.material) {
  ...
  update(deltaTime, playerPosition, moveInput) {
    if (this.globe) {
      ...
      this.globe.position.y = -620 + Math.sin(Date.now() * 35e-5) * 90;
      const distance = Math.abs(this.globe.position.z);
      const scaleFactor = Math.max(28, 28 + (22e3 - distance) / 320);
...
function updateCamera() {
  ...
  const offset = new Vector3(0, 15, -80);
